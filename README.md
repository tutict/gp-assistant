...
- 在全部样本外发布门槛形成版本化通过记录前，界面默认继续使用 legacy_balanced；默认路由只读取本地 SQLite 中 adaptive_swing_v1 的已通过报告。`internal_algorithm` 仅用于显式调试和发布验证，界面不暴露该参数，也不会改写默认发布状态。
...
- adaptive_swing_v1 回测会在同一份点时数据上同时运行旧 `balanced` walk-forward 对照，并返回发布门槛逐项结果；运行时从 SQLite 汇总当前实现过去 30 天内每个连续五次窗口的最低主榜+探索榜覆盖，以及未命中缓存/命中缓存的最慢端到端时延。任何证据未采集时门槛保持未通过；后续慢运行或低覆盖窗口会自动重算并撤销已有放行。
