...
- 在全部样本外发布门槛形成通过记录前，界面默认继续使用 legacy_balanced；内部设置 GP_ADAPTIVE_SWING_V1_ENABLED=1 后启用新版，也可用 internal_algorithm 在两套算法间强制切换，界面不暴露这些开关。
...
- adaptive_swing_v1 回测会在同一份点时数据上同时运行旧 `balanced` walk-forward 对照，并返回发布门槛逐项结果；五次主榜+探索榜覆盖和首次/缓存时延未采集时门槛保持未通过。

这些约束会降低“漂亮但不可复现”的回测结果，更适合用于筛选规则迭代和发布前验证。

从智能选股结果进入回测时，会携带完整的 adaptive_swing_v1 规格。每个调仓日重新截取当时可见的财务快照、行情、宽基指数和市场宽度后判断状态，禁止复用今天的市场状态或调仓日之后的数据。
