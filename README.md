...
“智能选股”默认使用 adaptive_swing_v1，面向 10–30 个交易日的波段研究。运行时先按财务质量、估值、流动性和数据完整度构建最多 80 只候选池，再结合上证综指、深证成指、创业板指与全市场宽度识别趋势、震荡、防守或过渡状态。概念标签只用于结果解释和探索榜分散，不直接加分。
...
- 内部可通过 internal_algorithm: "legacy_balanced" 回退旧筛选，界面不暴露该开关。
