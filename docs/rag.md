...
1. `policy_official`：中国政府网/国务院及公开部委政策列表，包含发改委、财政部、央行、证监会等官方政策原文入口；
2. `filing`：公告、财报、交易所或公司正式披露；
3. `financial_snapshot`：财务快照；
4. `news_media`：第一财经、财新等公开财经媒体 RSS/列表页，作为背景和市场解读；
5. `news`：其他可信新闻；
6. `research_report`：研报和用户导入资料；
7. `community`：社区讨论与传闻。

社区证据不能单独支撑事实结论。仅命中社区时保持证据模式，也不会调用聊天模型。配置聊天模型后，模型只能使用当前返回的 `[C1]` 等引用；缺失引用或生成未知引用编号时自动退回证据模式。

消息刷新会为政策/媒体消息增加 `scope_type`、`scope_tags` 和 `mapped_stock_codes`：范围分别为宏观、行业、区域和公司。公司范围按股票代码/名称/实体匹配，行业范围按自选股行业匹配，区域范围按自选股的省份/区域/城市字段匹配，宏观政策映射到当前自选股集合。也可以通过 `GP_NEWS_POLICY_URLS` 和 `GP_NEWS_MEDIA_URLS`（空格或分号分隔的公开 HTTPS URL）追加自定义公开源；单个源失败会进入刷新备注，不阻塞其它源。
